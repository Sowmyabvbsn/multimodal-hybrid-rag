...
from ingestion.offline_extract import OfflineChunk

class OfflineEmbeddingProcessor:
    """Process multimodal chunks and store embeddings in FAISS index - completely offline"""
    ...
    def process_chunk(self, chunk: OfflineChunk) -> Optional[np.ndarray]:
        ...
        try:
            if chunk.chunk_type == 'text':
                ...
                print(f"Unknown chunk type: {chunk.chunk_type}")
                return self.generate_text_embedding(chunk.content)
    ...
    def add_chunks(self, chunks: List[OfflineChunk]) -> int:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if embedding is not None and not np.isnan(embedding).any():
                ...
                print(f"Skipping chunk due to invalid embedding: {chunk.source_file}")
        ...
        print(f"Added {len(valid_chunks)} chunks to FAISS index")
